    ...
    def post(self, request):
        if request.method == "POST":
            ...
            if cursor.rowcount >= 1:
                cursor.execute("INSERT INTO ticket_Details(category,subcategory,product,hashtag,expiring_in_hours,budget_in_rs,scope,ticket_description,user_id,datecreated,dateupdated,interests_count) values(%s , %s ,%s , %s ,%s , %s ,%s , %s ,%s , %s ,%s, %s) RETURNING ticket_id",[category,subcategory,product,hashtag,expiring_in_hours,budget_in_rs,scope,ticket_description,user_id,datecreated,dateupdated,interests_count])
...
class GetMyTicketsAPI(APIView):
    ...
    def get(self, request):
        if request.GET.get('user_id') and request.GET.get('name') and request.GET.get('auth_key'):
            ...
            if cursor.rowcount >= 1:
                cursor.execute("SELECT ticket_id,category,subcategory,product,hashtag,expiring_in_hours,budget_in_rs,scope,ticket_description,datecreated,interests_count FROM ticket_details WHERE user_id = %s",[user_id])
                rows = cursor.fetchall()
                rowarray_list = []
                for row in rows:
...
class GetTicketById(APIView):
    ...
    def get(self, request):
        if request.GET.get('user_id') and request.GET.get('name') and request.GET.get('auth_key') and request.GET.get('ticket_id'):
            ...
            if cursor.rowcount >= 1:
                cursor.execute("SELECT ticket_id,category,subcategory,product,hashtag,expiring_in_hours,budget_in_rs,scope,ticket_description,datecreated,interests_count FROM ticket_details WHERE ticket_id = %s",[ticket_id])
                rows = cursor.fetchall()
                rowarray_list = []
                for row in rows:
                    t = (str(row[0]), row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8],str(row[9]), row[10])
...
class GetTicketByHashtag(APIView):
    ...
    def get(self, request):
        if request.GET.get('user_id') and request.GET.get('name') and request.GET.get('auth_key') and request.GET.get('hashtag'):
            ...
            if cursor.rowcount >= 1:
                cursor.execute("SELECT ticket_id,category,subcategory,product,hashtag,expiring_in_hours,budget_in_rs,scope,ticket_description,datecreated,interests_count FROM ticket_details WHERE hashtag = %s ORDER BY ticket_id desc",[hashtag])
                rows = cursor.fetchall()
                rowarray_list = []
                for row in rows:
                    t = (str(row[0]), row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8],str(row[9]),row[10])
...
class GetFewTicketByHashtag(APIView):
    ...
    def get(self, request):
        if request.GET.get('user_id') and request.GET.get('name') and request.GET.get('auth_key') and request.GET.get('hashtag'):
            ...
            if cursor.rowcount >= 1:
                cursor.execute("SELECT ticket_id,category,subcategory,product,hashtag,expiring_in_hours,budget_in_rs,scope,ticket_description,datecreated,interests_count FROM ticket_details WHERE hashtag = %s ORDER BY ticket_id DESC LIMIT 10",[hashtag])
                rows = cursor.fetchall()
                rowarray_list = []
                for row in rows:
                    t = (str(row[0]), row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8],str(row[9]),row[10])
